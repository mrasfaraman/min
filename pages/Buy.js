...
import {Evm_estimatedGas, Sol_estimatedGas} from '../utils/function';
import {
  ALERT_TYPE,
  Dialog,
...
const Buy = ({route, navigation}) => {
  ...
  const setMaxSol = async () => {
    setLoader(true);
    try {
      let gasData = await Sol_estimatedGas(
        activeAccount?.secretKey || activeAccount?.privateKey,
        recipientAddress || '8xpkMJkyhXMP2vydfQJKxZHZgyhSvqdjsqhQf23XBxvy',
        route?.params?.amount,
      );
      let maxBalance =
        Number(route?.params?.amount) - Number(gasData.gasFeeInEther);
      setAmountToSend(maxBalance.toString());
      setLoader(false);
    } catch (error) {
      setLoader(false);
      ...
      {loader ? (
        ...
        <>
          ...
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            ...
            />
            <TouchableOpacity
              onPress={activeNet?.type === 'solana' ? setMaxSol : setMax}
              style={{
                marginRight: 10,
